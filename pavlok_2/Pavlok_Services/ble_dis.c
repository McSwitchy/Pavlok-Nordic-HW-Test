...
uint32_t ble_dis_init(void)
{
  ...
  ble_dis_init_t	*	p_dis_init = &dis_init;
    ...
    {
   	(void)memset(&dis_init, 0, sizeof(dis_init));
    	ble_srv_ascii_to_utf8(&dis_init.manufact_name_str, (char *)PAVLOK_MANUFACTURER_NAME);
    	ble_srv_ascii_to_utf8(&dis_init.model_num_str, (char *)PAVLOK_MODEL_NUMBER);
    	ble_srv_ascii_to_utf8(&dis_init.serial_num_str, (char *)PAVLOK_SERIAL_NUMBER);
    	ble_srv_ascii_to_utf8(&dis_init.hw_rev_str, (char *)PAVLOK_HARDWARE_VERSION);
    	ble_srv_ascii_to_utf8(&dis_init.fw_rev_str, (char *)PAVLOK_FIRMWARE_VERSION);
    	ble_srv_ascii_to_utf8(&dis_init.sw_rev_str, (char *)PAVLOK_SOFTWARE_VERSION);
    ...
    // Add characteristics
    if ((err_code == NRF_SUCCESS)
    	&& (p_dis_init->manufact_name_str.length > 0))
    {
    ...
    if ((err_code == NRF_SUCCESS)
    	&& (p_dis_init->model_num_str.length > 0))
    {
    ...
    if ((err_code == NRF_SUCCESS)
    	&& (p_dis_init->serial_num_str.length > 0))
    {
    ...
    if ((err_code == NRF_SUCCESS)
    		&& (p_dis_init->hw_rev_str.length > 0))
    {
    ...
    if ((err_code == NRF_SUCCESS)
    		&& (p_dis_init->fw_rev_str.length > 0))
    {
    ...
    if ((err_code == NRF_SUCCESS)
    		&& (p_dis_init->sw_rev_str.length > 0))
    {
