...
ACC_MAG_TXRX_RET_T accel_mag_unlatch_int1(void)
{
	...
	if (accel_magnet_read(I2C_ACC_MAG_ADDRESS, FXOS8700CQ_TRANSIENT_SRC, &byte, 1) != ACC_MAG_SUCCESS) { return ACC_MAG_TX_ERROR; }
...
ACC_MAG_TXRX_RET_T accel_mag_unlatch_int2(void)
{
	...
	if (accel_magnet_read(I2C_ACC_MAG_ADDRESS, FXOS8700CQ_PULSE_SRC, &byte, 1) != ACC_MAG_SUCCESS) { return ACC_MAG_TX_ERROR; }
...
**	----------------------------------------------------------------------
*/
ACC_MAG_TXRX_RET_T accel_magnet_read(uint8_t i2c_addr, uint8_t addr, uint8_t *data, uint8_t length)
{
	ret_code_t errCode;
	accmagTXRXPacket_T accmagTXRXPacket;
	...
	memset(accmagTXRXPacket.data, 0, I2C_ACC_MAG_MAX_TRANSFER);
	accmagTXRXPacket.address = addr;

	errCode = i2c1_sync_read(i2c_addr, (uint8_t *)&accmagTXRXPacket, ACC_MAG_FXOS8700CQ_ADDRESS_SIZE, data, length);
...
**	----------------------------------------------------------------------
*/
int accelerometer_whoami(uint8_t address)
{
	int err;
	uint8_t byte = 0;

	err = accel_magnet_read(address, FXOS8700CQ_WHOAMI, &byte, 1);
...
ACC_MAG_TXRX_RET_T get_acceleration(ACCEL_DATA_T *accelData)
{
	...
	if (accel_magnet_read(I2C_ACC_MAG_ADDRESS, FXOS8700CQ_STATUS, buffer, SIZE_OF_ACC_DATA) != ACC_MAG_SUCCESS) { return ACC_MAG_TX_ERROR; }
...
ACC_MAG_INIT_RET_T accelerometer_init(void) {
	...
	ret = accelerometer_whoami(I2C_ACC_MAG_ADDRESS);
	if (ret) {
...
void verify_acc_mag_loop(void)
{
	...
	{
		nrf_delay_ms(200);
		testReturn = accelerometer_whoami(I2C_ACC_MAG_ADDRESS);
		if (testReturn)
		{
			UART_DEBUG("Success: FXOS8700CQ found on i2c bus\r\n");
		}
